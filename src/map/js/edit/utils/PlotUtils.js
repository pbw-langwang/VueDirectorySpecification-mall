...
export function wholeDistance(points) {
  ...
  for (var i = 0; i < points.length - 1; i++)
    distance1 += distance(points[i], points[i + 1]);
  return distance1;
}
...
export function getCurvePoints(t, controlPoints) {
  ...
  for (i = 0; i < controlPoints.length - 1; i++) {
    ...
    points.push(pnt1);
    for (var t1 = 0; t1 < Constants.FITTING_COUNT; t1++) {
      var pnt = getCubicValue(
        t1 / Constants.FITTING_COUNT,
        pnt1,
        normals[i * 2],
        normals[i * 2 + 1],
        pnt2
...
export function getLeftMostControlPoint(t, controlPoints) {
  ...
  var dist = Math.sqrt(normal[0] * normal[0] + normal[1] * normal[1]);
  if (dist > Constants.ZERO_TOLERANCE) {
    var midpoint = mid(pnt1, pnt2);
    var pX = pnt1[0] - midpoint[0];
    var pY = pnt1[1] - midpoint[1];
    ...
    var dX = normalRight[0] - midpoint[0];
    var dY = normalRight[1] - midpoint[1];

    // coordinates of reflected vector
    var controlX = midpoint[0] + a11 * dX + a12 * dY;
    var controlY = midpoint[1] + a12 * dX + a22 * dY;
  } else {
...
export function getRightMostControlPoint(t, controlPoints) {
  ...
  var dist = Math.sqrt(normal[0] * normal[0] + normal[1] * normal[1]);
  if (dist > Constants.ZERO_TOLERANCE) {
    var midpoint = mid(pnt2, pnt3);
    var pX = pnt3[0] - midpoint[0];
    var pY = pnt3[1] - midpoint[1];
    ...
    var dX = normalLeft[0] - midpoint[0];
    var dY = normalLeft[1] - midpoint[1];
